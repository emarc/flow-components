    ...
    public void templateFromRendererWithPeople() {
        ...
        for (int i = 0; i < items.size(); i++) {
            Assert.assertEquals(String.valueOf(i + 1),
                    ((ObjectNode) items.get(i)).get("key").asText());
            Assert.assertEquals("Person " + (i + 1), VirtualListHelpers
                    .getPropertyString((ObjectNode) items.get(i), "name"));
            Assert.assertEquals(String.valueOf(i + 1), VirtualListHelpers
                    .getPropertyString((ObjectNode) items.get(i), "age"));
            Assert.assertEquals("person_" + (i + 1), VirtualListHelpers
                    .getPropertyString((ObjectNode) items.get(i), "user"));
        }
        ...
        Assert.assertEquals("Person 1 Updated",
                VirtualListHelpers.getPropertyString(person, "name"));
        Assert.assertEquals("person_1_updated",
                VirtualListHelpers.getPropertyString(person, "user"));
    }
    ...
    private void assertItemsArePresent(ArrayNode items, int startingIndex,
        ...
        for (int i = startingIndex; i < endingIndex; i++) {
            Assert.assertFalse(
                    ...
                    items.get(i).isNull());
            Assert.assertEquals(itemLabelprefix + (i + 1), VirtualListHelpers
    ...
    private void clickToSet2Items_listIstUpdated(String listId,
        ...
        for (int i = 0; i < items.size(); i++) {
            Assert.assertEquals(
                    ...
                    VirtualListHelpers.getPropertyString(
                            (ObjectNode) items.get(i), "label"));
        }
    }
